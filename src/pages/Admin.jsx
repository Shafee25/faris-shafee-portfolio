...
const Admin = () => {
  ...
  // Form State - IMPORTANT: Default category is set here
  ...
  useEffect(() => {
    if (!user) return;
    setFormData({ category: 'AI Trends' }); // Reset form on tab change
    setIsEditing(false); 
  ...
  const insertFormat = (symbol, closeSymbol = "") => {
    ...
    const newText = `${text.substring(0, start)}${symbol}${text.substring(start, end)}${closeSymbol}${text.substring(end)}`;
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      const payload = { 
        ...formData, 
        author: formData.author || "Faris Shafee",
      ...
      setFormData({ category: 'AI Trends' }); // Reset form
      setIsEditing(false); 
      setEditId(null);
    } catch (err) {
      console.error(err);
  ...
  const handleDelete = async (id) => {
    if (confirm("Execute Delete Protocol?")) await deleteDoc(doc(db, activeTab, id));
  };
      ...
      case 'posts': // BLOG
        return (
          <>
            ...
            <div className="grid md:grid-cols-2 gap-4">
              <input name="author" placeholder="Author" value={formData.author || ''} onChange={handleInputChange} className="admin-input" />
              <input name="date" placeholder="Date" value={formData.date || ''} onChange={handleInputChange} className="admin-input" />
            </div>
            ...
            <textarea id="content-editor" name="content" placeholder="Write your article here..." value={formData.content || ''} onChange={handleInputChange} className="admin-input h-80 font-mono text-sm rounded-t-none mt-0" required />
          </>
        );
  ...
  // ==============================
  if (!user) return (
    <div className="min-h-screen flex items-center justify-center bg-[#0F1C20] relative overflow-hidden">
      <div className="absolute inset-0 z-0">
        <div className="absolute top-0 left-0 w-full h-full bg-[linear-gradient(to_right,#1f4e5620_1px,transparent_1px),linear-gradient(to_bottom,#1f4e5620_1px,transparent_1px)] bg-[size:4rem_4rem]"></div>
        <div className="absolute top-1/4 left-1/4 w-96 h-96 bg-highlight/10 rounded-full blur-[100px] animate-pulse"></div>
      ...
      >
        <div className="bg-[#0A1619]/80 backdrop-blur-xl border border-white/10 p-8 rounded-3xl shadow-2xl relative overflow-hidden group">
          <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-transparent via-highlight to-transparent opacity-50"></div>

          <div className="text-center mb-8">
            <div className="inline-flex items-center justify-center w-16 h-16 rounded-2xl bg-gradient-to-br from-secondary to-primary mb-4 shadow-lg border border-white/10">
              <ShieldCheck size={32} className="text-highlight" />
            </div>
            <h2 className="text-3xl font-bold text-white tracking-tight">System Access</h2>
            <p className="text-gray-400 text-sm mt-2 font-medium">Identify yourself, Admin.</p>
          </div>

          <form onSubmit={handleLogin} className="space-y-5">
            <div className="group/input">
              <label className="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-1 ml-1 group-focus-within/input:text-highlight transition-colors">Commander Email</label>
              <div className="relative">
                <div className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400 group-focus-within/input:text-white transition-colors"><User size={18} /></div>
                <input type="email" placeholder="admin@faris.dev" className="w-full bg-[#0F1C20] border border-[#1F4E56] text-white pl-12 pr-4 py-4 rounded-xl focus:outline-none focus:border-highlight focus:ring-1 focus:ring-highlight/50 transition-all shadow-inner placeholder-gray-600" onChange={e => setEmail(e.target.value)} />
              </div>
            </div>
            <div className="group/input">
              <label className="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-1 ml-1 group-focus-within/input:text-highlight transition-colors">Security Code</label>
              <div className="relative">
                <div className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400 group-focus-within/input:text-white transition-colors"><Lock size={18} /></div>
                <input type={showPassword ? "text" : "password"} placeholder="••••••••••••" className="w-full bg-[#0F1C20] border border-[#1F4E56] text-white pl-12 pr-12 py-4 rounded-xl focus:outline-none focus:border-highlight focus:ring-1 focus:ring-highlight/50 transition-all shadow-inner placeholder-gray-600" onChange={e => setPassword(e.target.value)} />
            ...
            <AnimatePresence>
              {loginError && (<motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="bg-red-500/10 border border-red-500/50 text-red-400 text-sm p-3 rounded-lg text-center font-medium">{loginError}</motion.div>)}
            </AnimatePresence>

            <button disabled={loading} className="w-full bg-gradient-to-r from-highlight to-orange-500 text-primary font-bold py-4 rounded-xl hover:shadow-[0_0_20px_rgba(252,163,17,0.4)] hover:scale-[1.02] active:scale-[0.98] transition-all duration-300 relative overflow-hidden">
              {loading ? ( <span className="flex items-center justify-center gap-2"><div className="w-5 h-5 border-2 border-primary border-t-transparent rounded-full animate-spin"></div>Authenticating...</span>) : ("Initialize Dashboard")}
            </button>
          </form>

          <div className="mt-6 text-center"><p className="text-gray-500 text-xs">Authorized Personnel Only • Secure Connection</p></div>
        </div>
      </motion.div>
    </div>
  );
    ...
    <div className="pt-24 pb-12 bg-primary min-h-screen px-4">
      <div className="max-w-7xl mx-auto">
        <div className="flex flex-wrap gap-4 justify-between items-center mb-8 bg-[#0A1619] p-6 rounded-2xl border border-[#1F4E56] shadow-xl relative overflow-hidden">
          ...
          <h1 className="text-2xl font-bold text-white flex items-center gap-3 relative z-10"><LayoutDashboard className="text-highlight" /> <span className="tracking-wide">COMMAND CENTER</span></h1>
        ...
        <div className="flex flex-wrap gap-3 mb-8">
          {[
            ...
            <button key={tab.id} onClick={() => setActiveTab(tab.id)} className={`flex items-center gap-2 px-6 py-3 rounded-xl font-bold transition-all border ${activeTab === tab.id ? 'bg-highlight text-primary border-highlight shadow-lg scale-105' : 'bg-[#0A1619] text-gray-400 border-[#1F4E56] hover:border-gray-500 hover:text-white'}`}><tab.icon size={18} /> {tab.label}</button>
          ))}
        </div>
        ...
        <div className="grid lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 bg-[#0A1619] border border-[#1F4E56] p-8 rounded-2xl h-fit shadow-xl relative">
            ...
            <form onSubmit={handleSave} className="space-y-5">
              ...
              <div className="flex gap-4 pt-4 border-t border-white/5 mt-6">
                {isEditing && <button type="button" onClick={() => {setIsEditing(false); setFormData({ category: 'AI Trends' });}} className="w-1/3 bg-gray-700 text-white font-bold py-3 rounded-xl hover:bg-gray-600 transition-all">Cancel</button>}
                <button disabled={loading} className={`flex-1 ${isEditing ? 'bg-blue-600 text-white' : 'bg-highlight text-primary'} font-bold py-3 rounded-xl hover:brightness-110 flex items-center justify-center gap-2 shadow-lg transition-all`}>
          ...
          <div className="lg:col-span-1 bg-[#0A1619] border border-[#1F4E56] p-6 rounded-2xl h-[650px] overflow-y-auto custom-scrollbar shadow-xl">
            <h2 className="text-xl font-bold text-white mb-4 flex items-center justify-between"><span>Database Records</span><span className="text-xs bg-white/10 px-2 py-1 rounded text-gray-300">{items.length}</span></h2>
            <div className="space-y-3">
              <AnimatePresence>
                {items.map(item => (
                  <motion.div initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 10 }} key={item.id} className={`bg-[#0F1C20] border ${editId === item.id ? 'border-highlight' : 'border-[#1F4E56]'} p-4 rounded-xl group hover:border-gray-500 transition-all relative overflow-hidden`}>
      ...
      <style>{`
        .admin-input { width: 100%; background-color: #0F1C20; border: 1px solid #1F4E56; color: white; padding: 14px; border-radius: 12px; outline: none; font-size: 0.95rem; transition: all 0.2s; }
        .admin-input:focus { border-color: #FCA311; box-shadow: 0 0 0 2px rgba(252, 163, 17, 0.1); background-color: #132429; }
        .toolbar-btn { padding: 8px; background: #1F4E56; color: #E0E6ED; border-radius: 6px; transition: all 0.2s; }
        .toolbar-btn:hover { background: #FCA311; color: #0F1C20; }
